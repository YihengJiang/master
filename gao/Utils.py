...
def proc(fname):
    #tmp=io.loadmat(fname,squeeze_me=True,struct_as_record=False)['x']
    # size = np.shape(tmp)
    # label = int(re.findall(".*_(\d+)\.mat", fname)[0]) - 1
    listData = []
...
def multiReadProc(files):
    ...
    pool = Pool(16, initializer=globalVarinit,
