...
sudo python setup.py install --prefix="/home/gaozf/.conda/envs/my_torch/"
'''
'''
import matlab.engine
eng = matlab.engine.start_matlab()
eng.addpath(eng.genpath('/home/gaozf/AP17/6_exp_pytorch/EER'))

def cal_eer_matlab(DScore, EScore, DKey, EKey):
    dScore = matlab.double(DScore.tolist())
    eScore = matlab.double(EScore.tolist())
    dKey = matlab.double((DKey+1).tolist())
    eKey = matlab.double((EKey+1).tolist())

    eer1, eer2 = eng.CalEER(dScore, eScore, dKey, eKey, nargout=2)
    return eer1, eer2
'''
'''
if __name__ == '__main__':
    # test
	EER = io.loadmat('./eer.mat')
    score = EER['score']
    key = EER['key']
    key = key - 1
    NumClass = EER['NumClass']
    print cal_eer(score, key, NumClass)
'''